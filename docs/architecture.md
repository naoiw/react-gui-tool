...
- **ポート一覧** … 接続可能なポートをプルダウンで表示し、1 つを選択する。
- **ボーレート** … よく使う値（9600, 19200, 38400, 57600, 115200 等）をプルダウンで選択する。
...
- **ポート一覧** … ユーザーが Refresh を押したときに `navigator.serial.getPorts()` 等で取得。設定の永続化は行わず、毎回選択する。
- **接続/切断** … 選択したポートとボーレートで `port.open()`、切断時は `port.close()`。
- **受信** … データが来るたびにイベント駆動で処理する。読み取りストリームからバイト列を受け取り、16byte 単位でパースする。
